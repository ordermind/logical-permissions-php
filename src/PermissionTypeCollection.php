    ...
    public function add(PermissionTypeInterface $permissionType, $overwriteIfExists = false)
    {
        ...
        if (!is_string($name)) {
            throw new InvalidPermissionTypeException(
                sprintf('The name of the permission type %s must be a string.', get_class($permissionType))
            );
        }
        if (!$name) {
            throw new InvalidPermissionTypeException(
                sprintf('The name of the permission type %s must not be empty.', get_class($permissionType))
            );
        }
        if (in_array(strtoupper($name), $reservedKeys = $this->getReservedPermissionKeys())) {
            throw new InvalidPermissionTypeException(
                sprintf(
                    'The permission type %s has the illegal name "%s". It must not be one of the following values: [%s]',
                    get_class($permissionType),
                    $name,
